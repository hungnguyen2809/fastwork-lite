...
export class FormatDate {
  ...
  public formatDate(time = Date.now()){
    ...
    if(time > -1){
      ...
      return `${this.addNumberZero(date.getDate())}/${this.addNumberZero(date.getMonth() + 1)}/${date.getFullYear()}`;
    }
    return null;
  }
  ...
  public formatTime(time = Date.now()){
    ...
    if(time > -1){
      const date = new Date(time);
      return `${this.addNumberZero(date.getHours())}:${this.addNumberZero(date.getMinutes())}`;
    }
    return null;
  }
